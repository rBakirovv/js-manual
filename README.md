# Материал по JS, React, паттернам проектирования 
...
[4.1 Паттерны проектирования](#паттерны-проектирования)
...
[5.2 Как работает браузер](#как-работает-браузер)
...
__Архитектура Flux__ + иммутабельные обновления:
...
__Flux__ следует однонаправленному потоку данных (unidirectional data flow), что делает поведение приложения более предсказуемым.

__Ключевые компоненты:__

- Actions (Действия) — описывают, что произошло (например, "USER_CLICKED_BUTTON").

- Dispatcher (Диспетчер) — центральный хаб, который распределяет actions всем stores.

- Stores (Хранилища) — содержат состояние и логику его обновления.

- Views (Представления) — React-компоненты, которые отображают данные из stores.
...
__Используйте интерфейсы (interface), если:__

- Работаете с классами (implements).

- Нужно расширять типы (extends).

- Хотите использовать declaration merging.

__Используйте типы (type), если:__

- Нужны объединения (|) или пересечения (&).

- Работаете с примитивами, кортежами или условными типами.

- Требуются вычисляемые свойства (mapped types).
...
__SOLID — это 5 принципов объектно-ориентированного дизайна, которые делают код гибким, понятным и поддерживаемым__
...
- Новый функционал добавляется без модификации существующего кода.
...
- Предотвращает неожиданное поведение в иерархии наследования.
...
- Упрощает замену реализаций (например, для тестов).

### FSD 

__FSD__ — это методология организации кода в React/Vue-приложениях, основанная на бизнес-фичах, а не на типах файлов.
...
- Упрощает навигацию в крупных проектах.

- Минимизирует "пропс-дриллинг".
...
__HTTP-запрос__ — это способ браузера или клиента общаться с сервером. Запрос состоит из нескольких частей:

- Метод запроса: наиболее распространённые — GET (запрос данных), POST (отправка данных), PUT, DELETE и другие.

- URL: адрес ресурса, к которому обращается клиент.

- Заголовки (Headers): передают метаданные о запросе (например, тип контента, куки, язык).

- Тело запроса (Body): присутствует чаще всего в POST-запросах и содержит данные для отправки на сервер.

- Ответ сервера также включает статус (например, 200 OK, 404 Not Found), заголовки ответа и тело (например, HTML, JSON, изображения).


### Как работает браузер

- Резолв DNS: сначала браузер преобразует доменное имя в IP-адрес через DNS.

- Установка TCP-соединения: создаётся соединение с сервером (часто через TCP и TLS для HTTPS).

- Отправка HTTP-запроса: браузер отправляет запрос серверу.

- Получение HTTP-ответа: сервер отвечает с данными.

- Парсинг HTML: браузер начинает разбирать полученный HTML-документ.

- Запросы дополнительных ресурсов: анализируются теги <link>, <script>, <img> и другие, выполняются дополнительные HTTP-запросы за CSS, JS, изображениями.

- Построение DOM, CSSOM, рендеринг: браузер строит дерево DOM, парсит стили в CSSOM, объединяет их в render tree и отображает визуально.

- Выполнение JavaScript: скрипты исполняются, что может повлиять на DOM и визуальное отображение.

- Обработка событий и взаимодействие с пользователем.

## Полезные материалы

Видео, где автор хорошо подготовлен к базовым и общим вопросам:

https://youtu.be/SToIrImHm4Y?si=MOxvKUkmGKIXdOlN
